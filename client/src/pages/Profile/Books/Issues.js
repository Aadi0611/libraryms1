...
function Issues({
    ...
    const onReturnHandler = async (issue) => {
        try {
            //if book is returned before due date
            const today = moment().format("YYYY-MM-DD hh:mm A");
            const dueDate = moment(issue.returnDate).format("YYYY-MM-DD hh:mm: A");
            if (today > dueDate) {
                //if book is returned after due date calculating the fine
                const fine = moment(today).diff(dueDate, "days") * 10;
                issue.fine = fine;
            }
